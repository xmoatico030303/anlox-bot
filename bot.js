...
client.on("guildMemberAdd", async member => {
  ...
  if(db.has(`sk_${member.guild.id}`) === false) return;
  if(db.has(`s_${member.guild.id}`) === false) return;
  const giriss = client.emojis.get('616074844060123297');
  
  const hgK = await db.fetch(`sk_${member.guild.id}`)
  if (!hgK) return;
  
  const s = db.fetch(`s_${member.guild.id}`)
  
  let vip = await db.fetch(`elmasüye_${member.id}`)
if (vip === 'elmas') {
  member.guild.channels.get(hgK).send(`<a:elmas:580817777699586070> :heavy_minus_sign::heavy_minus_sign::heavy_minus_sign::heavy_minus_sign::heavy_minus_sign: <a:elmas:580817777699586070> \n:loudspeaker: <a:yakut:580817540096458753> **Platinium **  \`${member.user.username}\` adlı **Platinium ** üye sunucuya katıldı! \`${db.fetch(`s_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`s_${member.guild.id}`) - member.guild.members.size}\` üye kaldı! \n<a:elmas:580817777699586070> :heavy_minus_sign::heavy_minus_sign::heavy_minus_sign::heavy_minus_sign::heavy_minus_sign: <a:elmas:580817777699586070>`);
} else if (vip === null) {
  member.guild.channels.get(hgK).send(`${giriss} :loudspeaker: :inbox_tray: \`${member.user.username}\` adlı kullanıcı sunucuya katıldı! \`${db.fetch(`s_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`s_${member.guild.id}`) - member.guild.members.size}\` üye kaldı!`)
}
});
...
if (vip === 'elmas') {
  ...
  member.guild.channels.get(hgK).send(`${anloxcikis} :loudspeaker: :inbox_tray: \`${member.user.username}\` adlı kullanıcı sunucuya Ayrıldı! \`${db.fetch(`s_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`s_${member.guild.id}`) - member.guild.members.size}\` üye kaldı!`)
}
});
/////
